...
void test_cache_aware_loading() {
    std::cout << "Testing cache-aware loading methods..." << std::endl;
    ...
    const std::string streets_path = "/tmp/test_streets.bin";
    const std::string osm_path = "/tmp/test_osm.bin";
    const std::string expected_cache_path = "/tmp/test_streets.gisevo.cache";
    
    std::cout << "Testing cache path generation..." << std::endl;
    const std::string generated_cache_path = db.generate_cache_path(streets_path, osm_path);
    assert(generated_cache_path == expected_cache_path);
    std::cout << "Cache path generation: " << generated_cache_path << std::endl;
    ...
    std::cout << "Testing cache save..." << std::endl;
    bool save_success = db.save_to_cache(expected_cache_path, cache_manager, streets_checksum, osm_checksum);
    assert(save_success);
    ...
    // Test cache load
    std::cout << "Testing cache load..." << std::endl;
    bool load_success = db.load_from_cache(expected_cache_path, cache_manager);
    assert(load_success);
    ...
    std::cout << "All cache-aware loading tests passed!" << std::endl;
    
    // Clean up
    std::remove(expected_cache_path.c_str());
}
...
int main() {
    try {
        gisevo::test_cache_aware_loading();
        std::cout << "Cache-aware loading test completed successfully!" << std::endl;
        return 0;
